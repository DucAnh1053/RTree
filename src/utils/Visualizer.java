...
public class Visualizer<T extends Boundable> {
		...
		while (!queue.isEmpty()) {
			...
			if (current != null) {
				if (current.getRecord() != null)
					...
					this.drawInternalNode(g, current, rootMbrWidthRange, rootMbrHeightRange, nodeHeight,
							treeHeight);
				// add next level
	...
	public void createVisualization(RTree<T> tree, Graphics2D g) {
		...
		float[] rootMbrHeightRange = new float[] { rootMbr.getBottomLeft().getY(), rootMbr.getTopLeft().getY() };
		// draw tree recursively
		this.drawNodes(tree.getRoot(), g, rootMbrWidthRange, rootMbrHeightRange, treeHeight);
		// g.dispose();
	}
	...
	public void createGridVisualization(List<Record<T>> records, List<Rectangle> grids, Rectangle treeBoundary,
		...
		graphics2D.fillRect(0, 0, this.image_size, this.image_size);
		// getting root's mbr limit dimensions
		float[] rootMbrWidthRange = new float[] { treeBoundary.getTopLeft().getX(),
				treeBoundary.getTopRight().getX() };
		...
		for (Rectangle grid : grids) {
			...
			graphics2D.draw(
					grid.draw(drawingDimensions[0], drawingDimensions[1], drawingDimensions[2],
							drawingDimensions[3]));
		}
		graphics2D.dispose();
		ImageIO.write(image, "png", filelocation);
